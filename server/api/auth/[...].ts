    ...
    CredentialsProvider.default({
      ...
      async authorize (credentials: any) {
        ...
        if (user) {
          ...
          const hash = (await pbkdf2(credentials.password, authSalt, 10000, 512, 'sha512')).toString('hex')

          if (credentials?.username === user.username && hash === user.password) {
            return { username: user.username, email: user.email, _count: user._count }
          }
        } else {
